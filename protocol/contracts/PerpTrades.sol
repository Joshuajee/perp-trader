...
contract PerpTrades is ERC4626, Ownable {
    ...
    event PositionSizeDecrease(uint indexed id, uint inc);
    ...
    function decreasePositionSize(uint id,  uint dec) external {
        ...
        if(positionSize == dec) {
            ...
            emit PositionSizeDecrease(id, dec);
        }
    }
    ...
    function _closePosition(uint _positionId) internal  {
        ...
        emit PositionClosed(_positionId, position.pair, position.trader, position.size);
